...
class DecorationButtonHandler(
    ...
    override fun onButtonInteraction(event: ButtonInteractionEvent) {
        ...
        if (args[0] != "deco") // If the component is not a warning component, return
            return
        ...
        val context = decorationStore.getDecorationRequest(UUID.fromString(processId))
        if (context == null) {
    ...
    override fun <T : RequestBase> acceptRequest(event: ButtonInteractionEvent, context: T) {
        ...
        val create = Create(
            ...
            "Request by $requesterId \n" +
                    "License: ${context.license ?: "None"}\n",
            context.imageUrl,
            arrayOf<String>()
        )

        requestManager.addRequest(create, object : ApiResponseHandler {
            override fun onSuccess(response: ApiResponse?) {
                val embedBuilder = EmbedBuilder(event.message.embeds[0])
                    .setColor(Color.GREEN)
                    .setDescription("Decoration has been approved.")
                event.message.editMessageEmbeds(embedBuilder.build()).queue()
                event.message.editMessageComponents().queue()

                decorationStore.approveDecorationRequest(context.requestId, event.member!!.idLong)
            }
    ...
    override fun <T : RequestBase> rejectRequest(event: ButtonInteractionEvent, context: T) {
        ...
        decorationStore.deleteDecorationRequest(context.requestId)
    }
}
