...
abstract class JsonApiRequest : ApiRequest {
    private val gson = Gson()
    private val jsonData: MutableMap<String, Any> = LinkedHashMap()

    protected fun add(key: String, value: Any?) {
        require(!StringUtils.isBlank(key)) { "Key cannot be blank" }
        if (value != null) jsonData[key] = value
    }
